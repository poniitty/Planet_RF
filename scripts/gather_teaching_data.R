...
# for(i in shps){
  ...
  if(nrow(p) > 2){
    ...
    areaid <- rev(strsplit(i, "/")[[1]])[3]
    
    if(!file.exists(paste0("extracted/", imageid,"_", areaid, ".csv"))){
      ...
      r_march <- rast(paste0(strsplit(i, "mosaiced")[[1]][1], "predictors/planet_march_median.tif"))
      r_march <- project(r_march, r)
      names(r_march) <- paste0("march_", c("blue","green","red","nir"))
      
      r_july <- rast(paste0(strsplit(i, "mosaiced")[[1]][1], "predictors/planet_july_median.tif"))
      ...
      if("L1" %in% names(st_coordinates(p) %>% as.data.frame())){
        p <- p %>% slice(st_coordinates(p) %>% as.data.frame() %>% pull(L1))
      }
      
      p <- bind_cols(st_drop_geometry(p),
                     terra::extract(rs, st_coordinates(p) %>% as.data.frame() %>% select(X,Y)) %>% select(-ID)) %>%
        filter(!is.na(blue)) %>%
        mutate(imageid = imageid)
      
      write_csv(p, paste0("extracted/", imageid,"_", areaid, ".csv"))
